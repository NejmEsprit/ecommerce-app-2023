...
export const getProductsCtrl = async (req, res) => {
    try {
        const prds = await productModel.find({}).select("-photo").populate('category').limit(12).sort({ createdAt: -1 })
        res.status(200).send({
            success: true,
            count_total: prds.length,
...
export const getSingleProductCtrl = async (req, res) => {
    try {
        ...
        const singleProduct = await productModel.findOne({ slug }).select("-photo").populate('category')
        res.status(200).send({
            success: true,
...
// product per page
export const productPerPageCtrl = async (req, res) => {
    try {
        const perPage = 3
...
// similar products
export const realtedProductCtrl = async (req, res) => {
    try {
...
export const productsByCategoryCtrl = async (req, res) => {
    try {
